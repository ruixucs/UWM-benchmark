...
class ModelArguments:
    ...
    detach_mm_projector: bool = field(default=False)
...
class LazySupervisedDataset_ImgGen(Dataset):
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            image_file = self.list_data_dict[i]['image']
            image_folder = self.data_args.image_folder
            if 'task' in sources[0] and sources[0]['task']=='generation':
                processor = self.data_args.image_processor_gen
            else:
                processor=self.data_args.image_processor
            image = Image.open(os.path.join(image_folder, image_file)).convert('RGB')
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            # image does not exist in the data, but the model is multimodal
            # this will skip in my_prepare_inputs_labels_for_multimodal, not input to the model
            data_dict['image'] = torch.zeros(self.data_args.image_shape_un)
        return data_dict
